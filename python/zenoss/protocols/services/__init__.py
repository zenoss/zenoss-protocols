...
class RestRequest(object):
    def __init__(self, uri, method="GET", headers=None, body=None):
        # Don't send unicode URLs - leads to UnicodeDecodeError in
        # Python 2.7 httplib.
...
class RestServiceClient(object):
    ...
    def __init__(
        self, uri, timeout=60, connection_error_class=ServiceConnectionError
    ):
        ...
        pool_key = (
            self._uri_parts.scheme,
            self._uri_parts.hostname,
            self._uri_parts.port,
        )
    ...
    def _buildRequest(
        self, path, method="GET", params={}, headers={}, body=None
    ):
        if self._default_headers:
            headers = dict(self._default_headers, **headers)
        return RestRequest(
            self.uri(path, params), method=method, headers=headers, body=body
        )
    ...
    def _executeRequest(self, request):
        try:
            ...
            # Convert response from urllib3 to httpLib2 for compatibility
            ...
            if e.errno == errno.ECONNREFUSED or e.errno == errno.EINVAL:
                raise self._connection_error_class(
        ...
        if not (response.status >= 200 and response.status <= 299):
            raise ServiceResponseError(
                response.reason, response.status, request, response, content
            )
    ...
    def _request_with_body(
        self, path, body=None, method="POST", params={}, headers={}
    ):
    ...
    def post(self, path, body, params={}, headers={}):
        return self._request_with_body(
            path, method="POST", body=body, params=params, headers=headers
        )

    def put(self, path, body, params={}, headers={}):
        return self._request_with_body(
            path, method="PUT", body=body, params=params, headers=headers
        )
    ...
    def delete(self, path, params={}, headers={}):
        return self._request(
            path, method="DELETE", params=params, headers=headers
        )
        ...
        if response.status == 200:
            if response["content-type"] != self._content_type:
                raise SerializeException(
                    'Expected "%s" got "%s"'
                    % (self._content_type, response["content-type"])
                )

            protobuf_type = response[self._protobuf_header.lower()]
            content = self._protobuf_schema.hydrateProtobuf(
                protobuf_type, content
            )
...
class ProtobufRestServiceClient(RestServiceClient):
    ...
    def __init__(
        ...
        uri,
        queueSchema,
        connection_error_class=ServiceConnectionError,
        **kwargs
    ):
        super(ProtobufRestServiceClient, self).__init__(
            uri, connection_error_class=connection_error_class, **kwargs
        )
...
class JsonRestServiceClient(RestServiceClient):
    def __init__(
        self, uri, connection_error_class=ServiceConnectionError, **kwargs
    ):
        super(JsonRestServiceClient, self).__init__(
            uri, connection_error_class=connection_error_class, **kwargs
        )
