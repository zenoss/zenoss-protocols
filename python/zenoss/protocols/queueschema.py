...
def substitute_replacements(template, replacements):
    ...
    def _replace(matchobj):
        ...
        raise MissingReplacementException(
            "Unable to replace %s in %s" % (matchobj.group(1), template)
        )
...
class Schema(object):
    ...
    def _getProperty(self, type_, identifier, key, default=None):
        ...
        return self._properties.get(
            ...
            self._properties.get(
                "{type_}.default.{key}".format(**locs), default
            ),
        )
    ...
    def _load(self, schema):
        for identifier, contentConfig in schema.get(
            "content_types", {}
        ).iteritems():
            content_type = ContentType(
                identifier, contentConfig["python_class"]
            )
            self._content_types[identifier] = content_type

        # exchanges
        for identifier, exchangeConfig in schema.get(
            "exchanges", {}
        ).iteritems():
    ...
    def _convertArguments(self, arguments):
        ...
        # TODO: amqplib doesn't support same types as Java client - for now
        # just use whatever types are returned from JSON parser
    ...
    def getExchange(self, name, replacements=None):
        ...
        compression = self._getExchangeProperty(
            exchange_node, "compression", "none"
        )

        return Exchange(
            exchange_node.identifier,
            ...
            [
                self._content_types[content_type_id]
                for content_type_id in exchange_node.content_type_ids
            ],
            substitute_replacements_in_arguments(
                exchange_node.arguments, replacements
            ),
            int(
                self._getExchangeProperty(
                    exchange_node.identifier, "delivery_mode", 2
                )
            ),
            compression.lower(),
        )
    ...
    def getQueue(self, name, replacements=None):
        ...
        arguments = substitute_replacements_in_arguments(
            queue_node.arguments, replacements
        )

        ttl = self._getQueueProperty(
            queue_node.identifier, "x-message-ttl", None
        )
        if ttl is not None:
            arguments["x-message-ttl"] = int(ttl)

        expires = self._getQueueProperty(
            queue_node.identifier, "x-expires", None
        )
        if expires is not None:
            arguments["x-expires"] = int(expires)

        queue = Queue(
            queue_node.identifier,
        ...
        for binding_node in queue_node.binding_nodes:
            binding = Binding(
                self.getExchange(
                    binding_node.exchange_identifier, replacements
                ),
                substitute_replacements(
                    binding_node.routing_key, replacements
                ),
                substitute_replacements_in_arguments(
                    binding_node.arguments, replacements
                ),
            )
            queue.bind(binding)
        ...
        if self._protobuf_full_name_to_class is None:
            ...
            for contentType in self._content_types.values():
                try:
                    ...
                    log.exception(
                        "Failed to resolve protobuf: %s", protobuf_name
                    )
        ...
        if protobuf_name in self._protobuf_full_name_to_class:
            ...
            try:
                ...
                raise SchemaException(
                    'Could not find protobuf "%s"' % protobuf_name
                )
            ...
            try:
                ...
                raise ImportError(
                    'Could not find protobuf python class "%s"'
                    % config.python_class
                )
    ...
    def hydrateProtobuf(self, protobuf_name, content):
        """
        Given serialized protobuf string and a name, this will return the
        hyrdated protobuf.
        """
