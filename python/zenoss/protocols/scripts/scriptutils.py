...
def addLoggingOptions(parser):
    parser.add_option(
        ...
        action="store_true",
        dest="quiet",
        help="No logging messages will be displayed",
        default=False,
    )
    parser.add_option(
        "-d",
...
def get_zenpack_schemas():
    """
    Loads ZenPack QJS schema files from the
    ZenPacks/<organization>/<name>/protocols/*.qjs files.
    """
    schemas = []
    for zp in pkg_resources.iter_entry_points("zenoss.zenpacks"):
        ...
        if zp.dist.resource_isdir(protocols_path):
            qjs_files = [
                f
                for f in zp.dist.resource_listdir(protocols_path)
                if f.endswith(".qjs")
            ]
            for qjs_file in qjs_files:
                ...
                try:
                    with closing(
                        zp.dist.get_resource_stream(__name__, qjs_path)
                    ) as f:
                        schemas.append(json.load(f))
                except Exception as e:
                    log.warn(
                        "Failed to load ZenPack schema %s: %s", qjs_file, e
                    )
    return schemas
