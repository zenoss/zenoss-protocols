...
from zenoss.protocols.scripts.scriptutils import (
    addLoggingOptions,
    get_zenpack_schemas,
    initLogging,
)
...
class Formatter(object):
    ...
    def dumpCommonHeaders(self, message, stream):
        ...
        if content_encoding is not None:
            self.dumpHeader(
                HEADER_ORIGINAL_CONTENT_ENCODING, content_encoding, stream
            )


class JsonFormatter(Formatter):
    """
    Outputs HTTP style headers with information about the queue state of
    the message along with a JSON formatted message.
    """
...
class ProtobufFormatter(Formatter):
    """
    Outputs HTTP style headers with information about the queue state of
    the message along with a base64 encoded protobuf message.
    """
    ...
    def read(self, schema, stream):
        while True:
            ...
            if r and r[0] == stream:
                ...
                if ch != "\xce":
                    raise FormatError(
                        "Received 0x%02x while expecting 0xce" % ord(ch)
                    )

                yield exchange, routing_key, schema.hydrateProtobuf(
                    type, payload
                )
...
class Dumper(object):
    def __init__(
        self, stream, formatter, channel, schema, acknowledge=False, skip=False
    ):
    ...
    def dumpQueue(self, queueName, limit=None):
        ...
        if msg:
            ...
            log.info(
                "Receiving messages from queue: %s (%d available)",
                queueName,
...
def main():
    ...
    parser.add_option(
        ...
        action="store_true",
        dest="acknowledge",
        help="Acknowledge the message, acknowledging a message will "
    ...
    parser.add_option(
        ...
        action="store_true",
        dest="skip",
        help="Skip processing messages on the queue - use "
        "with --ack to clear a queue.",
    )
