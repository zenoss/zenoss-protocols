...
class IExchange(Interface):
    identifier = Attribute(
        "The unique identifier for the exchange in the queue schema."
    )
    name = Attribute("The name of the exchange.")
    type = Attribute("The type of exchange (e.g. topic, fanout, headers).")
    durable = Attribute(
        "True if the exchange is durable (persists after restart)."
    )
    auto_delete = Attribute(
        "True if the exchange should be deleted when it is no longer used "
        "(Deprecated)."
    )
    description = Attribute("An optional description of the exchange.")
    content_types = Attribute(
        "The content types (IContentType) which are published to the exchange."
    )
    arguments = Attribute(
        "The optional arguments used to declare the exchange."
    )


class IQueue(Interface):
    identifier = Attribute(
        "The unique identifier for the queue in the queue schema."
    )
    name = Attribute("The name of the queue.")
    durable = Attribute(
        "True if the queue is durable (persists after restart)."
    )
    exclusive = Attribute(
        "True if only one consumer is allowed to consume from the queue."
    )
    auto_delete = Attribute(
        "True if the queue should be deleted when it is no longer used."
    )
    description = Attribute("An optional description of the queue.")
    bindings = Attribute(
        "All defined bindings (IBinding) of the queue to exchanges."
    )
...
class IQueueSchema(Interface):
    """
    Interface to retrieve defined queues, exchanges, and content types
    (typically loaded from a .qjs file).
    """

    def getContentType(identifier):
        """
        Returns the content type with the specified identifier,
    ...
    def getExchange(identifier, replacements=None):
        """
        Returns the exchange with the specified identifier,
        or None if it doesn't exist.

        @type identifier: str
        @param identifier: The unique identifier for the exchange.
        @type replacements: dict
        @param replacements: An optional dictionary containing replacement
            values to be substituted in the exchange configuration.
        @rtype: IExchange
    ...
    def getQueue(identifier, replacements=None):
        """
        Returns the queue with the specified identifier,
        or None if it doesn't exist.
        ...
        @type replacements: dict
        @param replacements: An optional dictionary containing replacement
            values to be substituted in the queue configuration.
        @rtype: IQueue
    ...
    def getProtobuf(protobuf_name):
        """
        Returns the protobuf class matching the given content type identifier
        or protobuf message full name.

        @type protobuf_name: str
        @param protobuf_name: The content type unique identifier or
            the protobuf message full name.
    ...
    def getNewProtobuf(protobuf_name):
        """
        Returns a newly instantiated protobuf message for the given content
        type identifier or protobuf message full name.
    ...
    def hydrateProtobuf(protobuf_name, content):
        """
        Returns a populated protobuf message with the specified content
        type identifier or protobuf message full name and encoded protobuf.

        @type protobuf_name: str
        @param protobuf_name: The content type unique identifier or
            the protobuf message full name.
        @type content: str
        @param content: The encoded protobuf message (as received
            on a queue for example).
        @rtype: google.protobuf.Message
...
class IAMQPConnectionInfo(Interface):
    ...
    amqpconnectionheartbeat = Attribute(
        "AMQP Connection Heart Beat in Seconds"
    )


class IAMQPChannelAdapter(Interface):
    """
    Used to adapt the appropriate AMQP channel implementation and provide a
    way to declare a queue or exchange using the IQueue or IExchange returned
    from the queue schema.
    """
